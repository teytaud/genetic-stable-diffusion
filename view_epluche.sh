...
echo '\hline'
grep '^L' archive_results.txt | sed 's/ / \& /g' | sed 's/$/\\\\/g' | cut -c 3- | sed 's/_/-/g' | awk '{ if ( $1 != prev ) { print "\\hline" ; print "\\end{tabular}\\end{minipage} \\\\ \\begin{minipage}{.33\\textwidth}\\begin{tabular}{|c|c|c|c|}\\hline" } print $0; prev = $1 }'
echo '\hline'
...
echo '\hline'
grep '^O' archive_results.txt | sed 's/ / \& /g' | sed 's/$/\\\\/g' | cut -c 3- | sed 's/_/-/g' | awk '{ if ( $1 != prev ) { print "\\hline" ; print "\\end{tabular}\\end{minipage} \\\\ \\begin{minipage}{.33\\textwidth}\\begin{tabular}{|c|c|c|c|}\\hline" } print $0; prev = $1 }' | sed 's/[M-j]ajority/Random/g'
echo '\hline'
echo '\end{tabular}'
echo '\end{minipage}'
echo '\end{scriptsize}'
